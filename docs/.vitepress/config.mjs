import { defineConfig } from "vitepress";

export default defineConfig({
  title: "OpenAssetTools",
  description: "Documentation for and around OpenAssetTools",
  head: [["link", { rel: "icon", href: "/logo_64_64.png" }]],
  themeConfig: {
    logo: "/logo_64_64.png",
    outline: {
      level: "deep",
    },
    nav: [
      { text: "Home", link: "/" },
      {
        text: "Guide",
        link: "/guide/what-is-oat",
        activeMatch: "\\/guide(?:\\/|$)",
      },
      {
        text: "Reference",
        link: "/reference/zone-file",
        activeMatch: "\\/reference(?:\\/|$)",
      },
      {
        text: "Game Assets",
        link: "/asset/overview",
        activeMatch: "\\/asset(?:\\/|$)",
      },
    ],

    footer: {
      message:
        "All trademarks are owned by their respective owners. This project is neither endorsed by, nor affiliated with Activision, Treyarch, Infinity Ward or any of their descendants.",
    },

    sidebar: {
      "/guide": [
        {
          text: "Guides",
          items: [
            { text: "What is OAT?", link: "/guide/what-is-oat" },
            { text: "Getting started", link: "/guide/getting-started" },
            { text: "Using the tools", link: "/guide/using-the-tools" },
          ],
        },
        {
          text: "OAT Concepts",
          items: [
            { text: "Folder structure", link: "/guide/folder-structure" },
            { text: "OAT components", link: "/guide/components" },
          ],
        },
      ],
      "/reference": [
        {
          text: "OAT Reference",
          items: [
            { text: "Zone Files", link: "/reference/zone-file" },
            {
              text: "The IPak file format",
              link: "/reference/ipak-file-format",
            },
            {
              text: "Preprocessor features",
              link: "/reference/preprocessor-features",
            },
          ],
        },
      ],
      "/asset": [
        {
          text: "Game Assets",
          items: [
            {
              text: "Overview",
              collapsed: true,
              link: "/asset/overview",
              items: [
                {
                  text: "Asset Types",
                  collapsed: true,
                  link: "/asset/types",
                  items: [
                    {
                      text: "Anims",
                      collapsed: true,
                      link: "/asset/types/anim/",
                      items: [],
                    },
                    {
                      text: "DDL",
                      collapsed: true,
                      link: "/asset/types/ddl/",
                      items: [],
                    },
                    {
                      text: "Destructibles",
                      collapsed: true,
                      link: "/asset/types/destructible/",
                      items: [],
                    },
                    {
                      text: "Emblem Sets",
                      collapsed: true,
                      link: "/asset/types/emblem-set/",
                      items: [],
                    },
                    {
                      text: "Fonts",
                      collapsed: true,
                      link: "/asset/types/font/",
                      items: [{ text: "Font Icons" }, { text: "Text Fonts" }],
                    },
                    {
                      text: "Footsteps",
                      collapsed: true,
                      link: "/asset/types/footstep/",
                      items: [
                        { text: "Footstep Sound Tables" },
                        { text: "Footstep FX Tables" },
                      ],
                    },
                    {
                      text: "FX",
                      collapsed: true,
                      link: "/asset/types/fx/",
                      items: [{ text: "Impact FX" }, { text: "Standard FX" }],
                    },
                    {
                      text: "Glasses",
                      collapsed: true,
                      link: "/asset/types/glass/",
                      items: [],
                    },
                    {
                      text: "Images",
                      collapsed: true,
                      link: "/asset/types/image/",
                      items: [],
                    },
                    {
                      text: "Leaderboards",
                      collapsed: true,
                      link: "/asset/types/leaderboard/",
                      items: [],
                    },
                    {
                      text: "Localizations",
                      collapsed: true,
                      link: "/asset/types/locale/",
                      items: [{ text: "Supported Languages" }],
                    },
                    {
                      text: "Maps",
                      collapsed: true,
                      link: "/asset/types/map/",
                      items: [
                        { text: "Clipmap" },
                        { text: "Comworld" },
                        {
                          text: "Gameworld",
                        },
                        {
                          text: "Mapents",
                        },
                        { text: "Gfxworld" },
                        { text: "Lightdef" },
                        { text: "UImap" },
                        { text: "Addonmapents" },
                      ],
                    },
                    {
                      text: "Materials",
                      collapsed: true,
                      link: "/asset/types/material/",
                      items: [],
                    },
                    {
                      text: "Menus",
                      collapsed: true,
                      link: "/asset/types/menu/",
                      items: [
                        {
                          text: "LUI",
                          collapsed: true,
                          link: "/asset/types/menu/lui/",
                          items: [
                            {
                              text: "Structure",
                              link: "/asset/types/menu/lui/structure",
                            },
                            {
                              text: "Menu Properties",
                              link: "/asset/types/menu/lui/menu-properties",
                            },
                            {
                              text: "Event Handlers",
                              link: "/asset/types/menu/lui/event-handlers",
                            },
                          ],
                        },
                        {
                          text: "Quake",
                          collapsed: true,
                          link: "/asset/types/menu/quake/",
                          items: [
                            {
                              text: "Structure",
                              link: "/asset/types/menu/quake/structure",
                            },
                            {
                              text: "Preprocessor",
                              link: "/asset/types/menu/quake/preprocessor",
                            },
                            {
                              text: "Menu Properties",
                              link: "/asset/types/menu/quake/menu-properties",
                            },
                            {
                              text: "Item Properties",
                              link: "/asset/types/menu/quake/item-properties",
                            },
                            {
                              text: "Expressions",
                              link: "/asset/types/menu/quake/expressions",
                            },
                            {
                              text: "Event Handlers",
                              link: "/asset/types/menu/quake/event-handlers",
                            },
                          ],
                        },
                      ],
                    },
                    {
                      text: "Model Pieces",
                      collapsed: true,
                      link: "/asset/types/model-pieces/",
                      items: [],
                    },
                    {
                      text: "Models",
                      collapsed: true,
                      link: "/asset/types/model/",
                      items: [{ text: "Collision Maps" }],
                    },
                    {
                      text: "Physics Constraints",
                      collapsed: true,
                      link: "/asset/types/phys-constraint/",
                      items: [],
                    },
                    {
                      text: "Physics Presets",
                      collapsed: true,
                      link: "/asset/types/phys-preset/",
                      items: [],
                    },
                    {
                      text: "QDB",
                      collapsed: true,
                      link: "/asset/types/qdb/",
                      items: [],
                    },
                    {
                      text: "Rawfiles",
                      collapsed: true,
                      link: "/asset/types/rawfile/",
                      items: [
                        { text: "LUI Scripts" },
                        { text: "Configs" },
                        { text: "GSC Scripts" },
                        { text: "Graphs" },
                        { text: "Animstatedefs" },
                        { text: "Animtrees" },
                        { text: "Menu Expressions" },
                        { text: "Penetration Tables" },
                        { text: "Location Damage Tables" },
                        { text: "Gametype Definitions" },
                        { text: "Player Anim Scripts" },
                        { text: "Player Anim Types" },
                        { text: "Radiant Script Keys" },
                        { text: "Rumbles" },
                        { text: "Shaders" },
                        { text: "Shocks" },
                        { text: "Suns" },
                        { text: "Atari 2600 Games" },
                        { text: "Visions" },
                        { text: "Zone Ending Files" },
                        { text: "Arenas" },
                      ],
                    },
                    {
                      text: "Script Parse Trees",
                      collapsed: true,
                      link: "/asset/types/script/",
                      items: [],
                    },
                    {
                      text: "Slugs",
                      collapsed: true,
                      link: "/asset/types/slug/",
                      items: [],
                    },
                    {
                      text: "Skinned Vertices",
                      collapsed: true,
                      link: "/asset/types/skinned-verts/",
                      items: [],
                    },
                    {
                      text: "Sounds",
                      collapsed: true,
                      link: "/asset/types/sound/",
                      items: [
                        { text: "Streamed Sounds" },
                        { text: "Loaded Sounds" },
                        { text: "Sound Patches" },
                        { text: "Sound Driver Globals" },
                      ],
                    },
                    {
                      text: "Stringtables",
                      collapsed: true,
                      link: "/asset/types/stringtable/",
                      items: [],
                    },
                    {
                      text: "Technique Sets",
                      collapsed: true,
                      link: "/asset/types/techset/",
                      items: [],
                    },
                    {
                      text: "Tracers",
                      collapsed: true,
                      link: "/asset/types/tracer/",
                      items: [],
                    },
                    {
                      text: "Vehicles",
                      collapsed: true,
                      link: "/asset/types/vehicle/",
                      items: [],
                    },
                    {
                      text: "Weapons",
                      collapsed: true,
                      link: "/asset/types/weapon/",
                      items: [
                        { text: "Attachment Systems" },
                        { text: "Weapon Definitions" },
                        { text: "Weapon Camo Systems" },
                      ],
                    },
                    {
                      text: "ZBarriers",
                      collapsed: true,
                      link: "/asset/types/zbarrier/",
                      items: [],
                    },
                  ],
                },
                {
                  text: "Asset Formats",
                  collapsed: true,
                  link: "/asset/formats",
                  items: [
                    {
                      text: "Animations",
                      collapsed: true,
                      link: "/asset/formats/anim/",
                      items: [
                        { text: "Cod Binary" },
                        { text: "Cod Source" },
                        { text: "Cod SEAnim" },
                      ],
                    },
                    {
                      text: "Config String Parse",
                      collapsed: true,
                      link: "/asset/formats/csp/",
                      items: [
                        { text: "Attachment Unique" },
                        { text: "Attachment" },
                        { text: "Bullet Penetration Table" },
                        { text: "Destructible" },
                        { text: "Generic CSP" },
                        { text: "Location Damage Table" },
                        { text: "Physics Constraint" },
                        { text: "Physics Preset" },
                        { text: "Rumble" },
                        { text: "Tracer" },
                        { text: "Vehicle" },
                        { text: "Weapon" },
                        { text: "ZBarrier" },
                      ],
                    },
                    {
                      text: "CSV",
                      collapsed: true,
                      link: "/asset/formats/csv/",
                      items: [
                        {
                          text: "Sound",
                          collapsed: true,
                          link: "/asset/formats/csv/sound/",
                          items: [
                            {
                              text: "Globals",
                              collapsed: true,
                              link: "/asset/formats/csv/sound/globals/",
                              items: [
                                { text: "Build Pack" },
                                { text: "Context" },
                                { text: "Curves" },
                                { text: "Effects" },
                                { text: "Freeverb" },
                                { text: "Futz" },
                                { text: "Generic Globals" },
                                { text: "Group" },
                                { text: "Master" },
                                { text: "Metadata" },
                                { text: "Pan" },
                                { text: "Patch" },
                                { text: "Radverb" },
                                { text: "Reverb" },
                                { text: "Snapshot Groups" },
                                { text: "Snapshot" },
                                { text: "Zone Build" },
                                { text: "Zone Snapshot" },
                                { text: "Zone Sound" },
                                { text: "Zone Verb" },
                                { text: "Zones" },
                              ],
                            },
                            { text: "Sound Aliases" },
                            { text: "Sound Driver Globals" },
                            { text: "Sound Patch" },
                          ],
                        },
                        {
                          text: "Zone Source",
                          collapsed: true,
                          link: "/asset/formats/csv/zone-source/",
                          items: [
                            {
                              text: "Asset Info",
                              collapsed: true,
                              link: "/asset/formats/csv/zone-source/assetinfo/",
                              items: [
                                { text: "Generic Depenencies" },
                                { text: "Generic Zone" },
                              ],
                            },
                            {
                              text: "Asset List",
                              collapsed: true,
                              link: "/asset/formats/csv/zone-source/assetlist/",
                              items: [
                                { text: "Ignore Files" },
                                { text: "Generic Zone" },
                              ],
                            },
                            { text: "Zone" },
                          ],
                        },
                        { text: "Attachment Table" },
                        { text: "Attributes Table" },
                        { text: "Clan Tag Features" },
                        { text: "Country Table" },
                        { text: "Faction Table" },
                        { text: "Footstep FX Table" },
                        { text: "Footstep Sound Table" },
                        { text: "Gametypes Table" },
                        { text: "Generic CSV" },
                        { text: "Glass" },
                        { text: "Glasses" },
                        { text: "Impact FX" },
                        { text: "Leaderboard" },
                        { text: "Maps Table" },
                        { text: "Rank Table" },
                        { text: "Reflections" },
                        { text: "Stats Table" },
                        { text: "Texture List" },
                        { text: "VS Mode Class Table" },
                        { text: "Weapon Options" },
                      ],
                    },
                    { text: "DDL" },
                    {
                      text: "GDT",
                      collapsed: true,
                      link: "/asset/formats/gdt/",
                      items: [
                        {
                          text: "Material",
                          collapsed: true,
                          link: "/asset/formats/gdt/material",
                          items: [
                            { text: "Material Factory" },
                            { text: "Material Properties" },
                            { text: "Material Template" },
                            { text: "Material" },
                          ],
                        },
                        {
                          text: "Weapon",
                          collapsed: true,
                          link: "/asset/formats/gdt/weapon/",
                          items: [
                            { text: "Bullet Weapon" },
                            { text: "Dual-Wield Projectile Weapon" },
                            { text: "Dual-Wield Weapon" },
                            { text: "Gas Weapon" },
                            { text: "Generic Weapon" },
                            { text: "Grenade Weapon" },
                            { text: "Projectile Weapon" },
                            { text: "Turret Weapon" },
                            { text: "Vehicle Weapon" },
                            { text: "Weapon Camo" },
                          ],
                        },
                        { text: "AI Type" },
                        { text: "Bullet Penetration" },
                        { text: "Character" },
                        { text: "CSP" },
                        { text: "Destructible" },
                        { text: "Destructible Piece" },
                        { text: "Emblem" },
                        { text: "Flametable" },
                        { text: "Generic GDT" },
                        { text: "Glass" },
                        { text: "Level" },
                        { text: "Light" },
                        { text: "Location Damage Table" },
                        { text: "Multiplayer Body" },
                        { text: "Multiplayer Head" },
                        { text: "Multiplayer TYpe" },
                        { text: "Physics Constraint" },
                        { text: "Physics Preset" },
                        { text: "Pimp" },
                        { text: "Rumble" },
                        { text: "Vehicle Physics Parameters" },
                        { text: "Vehicle" },
                        { text: "Anim" },
                        { text: "Model" },
                        { text: "Model Alias" },
                      ],
                    },
                    {
                      text: "GSC",
                      collapsed: true,
                      link: "/asset/formats/gsc/",
                      items: [
                        {
                          text: "Animation Scripts",
                          collapsed: true,
                          link: "/asset/formats/gsc/animscripts/",
                          items: [
                            {
                              text: "Big Dog",
                              collapsed: true,
                              link: "/asset/formats/gsc/animscripts/bigdog/",
                              items: [
                                { text: "Combat" },
                                { text: "Death" },
                                { text: "Flashed" },
                                { text: "Init" },
                                { text: "Move" },
                                { text: "Pain" },
                                { text: "Scripted" },
                                { text: "Stop" },
                              ],
                            },
                            {
                              text: "Dog",
                              collapsed: true,
                              link: "/asset/formats/gsc/animscripts/dog/",
                              items: [
                                { text: "Combat" },
                                { text: "Death" },
                                { text: "Flashed" },
                                { text: "Init" },
                                { text: "Jump" },
                                { text: "Move" },
                                { text: "Pain" },
                                { text: "Scripted" },
                                { text: "Stop" },
                                { text: "Turn" },
                              ],
                            },
                            {
                              text: "Human",
                              collapsed: true,
                              link: "/asset/formats/gsc/animscripts/human/",
                              items: [
                                {
                                  text: "MG42",
                                  collapsed: true,
                                  link: "/asset/formats/gsc/animscripts/human/mg42/",
                                  items: [
                                    { text: "Common" },
                                    { text: "Crouch" },
                                    { text: "Prone" },
                                    { text: "Stand" },
                                  ],
                                },
                                {
                                  text: "SAW",
                                  collapsed: true,
                                  link: "/asset/formats/gsc/animscripts/human/saw/",
                                  items: [
                                    { text: "Common" },
                                    { text: "Crouch" },
                                    { text: "Prone" },
                                    { text: "Stand" },
                                  ],
                                },
                                {
                                  text: "Technical",
                                  collapsed: true,
                                  link: "/asset/formats/gsc/animscripts/human/technical/",
                                  items: [
                                    { text: "Common" },
                                    { text: "Stand" },
                                  ],
                                },
                                {
                                  text: "Turret",
                                  collapsed: true,
                                  link: "/asset/formats/gsc/animscripts/human/turret/",
                                  items: [
                                    { text: "Crouch" },
                                    { text: "Prone" },
                                    { text: "Stand" },
                                    { text: "Turret" },
                                  ],
                                },
                                { text: "Anim Mode" },
                                { text: "Combat" },
                                { text: "Concealment Crouch" },
                                { text: "Concealment Prone" },
                                { text: "Concealment Stand" },
                                { text: "Cover Arrival" },
                                { text: "Cover Behavior" },
                                { text: "Cover Crouch" },
                                { text: "Cover Left" },
                                { text: "Cover Pillar" },
                                { text: "Cover Prone" },
                                { text: "Cover Right" },
                                { text: "Cover Stand" },
                                { text: "Cover Wall" },
                                { text: "Cover Wide Left" },
                                { text: "Cover Wide Right" },
                                { text: "Death" },
                                { text: "First Frame" },
                                { text: "Flashed" },
                                { text: "Grenade Cower" },
                                { text: "Grenade Return Throw" },
                                { text: "Init" },
                                { text: "Melee" },
                                { text: "Move" },
                                { text: "Pain" },
                                { text: "Prone" },
                                { text: "React" },
                                { text: "Scripted" },
                                { text: "Stop" },
                                { text: "Turn" },
                              ],
                            },
                            {
                              text: "Zombie",
                              collapsed: true,
                              link: "/asset/formats/gsc/animscripts/zombie/",
                              items: [
                                { text: "Combat" },
                                { text: "Death" },
                                { text: "Init" },
                                { text: "Jump" },
                                { text: "Move" },
                                { text: "Pain" },
                                { text: "Scripted" },
                                { text: "Stop" },
                              ],
                            },
                            {
                              text: "Zombie Dog",
                              collapsed: true,
                              link: "/asset/formats/gsc/animscripts/zombie-dog/",
                              items: [
                                { text: "Combat" },
                                { text: "Death" },
                                { text: "Flashed" },
                                { text: "Init" },
                                { text: "Move" },
                                { text: "Pain" },
                                { text: "Scripted" },
                                { text: "Stop" },
                              ],
                            },
                          ],
                        },
                        {
                          text: "Faction",
                          collapsed: true,
                          link: "/asset/formats/gsc/faction/",
                          items: [
                            { text: "Multiplayer Body" },
                            { text: "Team Set" },
                          ],
                        },
                        {
                          text: "Gametype",
                          collapsed: true,
                          link: "/asset/formats/gsc/gametype/",
                          items: [{ text: "Gametype" }],
                        },
                        {
                          text: "Map",
                          collapsed: true,
                          link: "/asset/formats/gsc/map/",
                          items: [
                            { text: "Create Art" },
                            { text: "Create Cam" },
                            { text: "Create FX" },
                            { text: "Map Ambiance" },
                            { text: "Map Animations" },
                            { text: "Map FX" },
                            { text: "Map" },
                          ],
                        },
                        { text: "AI Type" },
                        { text: "Character" },
                        { text: "Code Callbacks" },
                        { text: "Delete" },
                        { text: "Generic CSC" },
                        { text: "Generic GSC" },
                        { text: "Multiplayer Body" },
                        { text: "Multiplayer Head" },
                        { text: "Multiplayer Type" },
                        { text: "Struct" },
                        { text: "Model Alias" },
                        {
                          text: "Weapon",
                          collapsed: true,
                          link: "/asset/formats/gdt/weapon/",
                          items: [
                            { text: "Bullet Weapon" },
                            { text: "Dual-Wield Projectile Weapon" },
                            { text: "Dual-Wield Weapon" },
                            { text: "Gas Weapon" },
                            { text: "Generic Weapon" },
                            { text: "Grenade Weapon" },
                            { text: "Projectile Weapon" },
                            { text: "Turret Weapon" },
                            { text: "Vehicle Weapon" },
                            { text: "Weapon Camo" },
                          ],
                        },
                        { text: "AI Type" },
                        { text: "Bullet Penetration" },
                        { text: "Character" },
                        { text: "CSP" },
                        { text: "Destructible" },
                        { text: "Destructible Piece" },
                        { text: "Emblem" },
                        { text: "Flametable" },
                        { text: "Generic GDT" },
                        { text: "Glass" },
                        { text: "Level" },
                        { text: "Light" },
                        { text: "Location Damage Table" },
                        { text: "Multiplayer Body" },
                        { text: "Multiplayer Head" },
                        { text: "Multiplayer TYpe" },
                        { text: "Physics Constraint" },
                        { text: "Physics Preset" },
                        { text: "Pimp" },
                        { text: "Rumble" },
                        { text: "Vehicle Physics Parameters" },
                        { text: "Vehicle" },
                        { text: "Anim" },
                        { text: "Model" },
                        { text: "Model Alias" },
                      ],
                    },
                    { text: "FX" },
                    { text: "Emblem Set" },
                    { text: "Font Icon" },
                    { text: "Font" },
                    { text: "Havok Script" },
                    { text: "IWI" },
                    { text: "Locale" },
                    {
                      text: "Map",
                      collapsed: true,
                      link: "/asset/formats/map/",
                      items: [
                        {
                          text: "BSP",
                        },
                        {
                          text: "Map Source",
                          collapsed: true,
                          link: "/asset/formats/map/map-source/",
                          items: [
                            { text: "Model Collision" },
                            { text: "Map Source" },
                          ],
                        },
                        { text: "Exposure" },
                        { text: "Map Entities" },
                      ],
                    },
                    {
                      text: "Model",
                      collapsed: true,
                      link: "/asset/formats/model/",
                      items: [
                        { text: "Cod Binary" },
                        { text: "Cod Source" },
                        { text: "Cod SEModel" },
                      ],
                    },
                    { text: "QDB" },
                    {
                      text: "Quake Menu",
                      collapsed: true,
                      link: "/asset/formats/quake-menu/",
                      items: [
                        { text: "Include" },
                        { text: "Menu Definition" },
                        { text: "Menu List" },
                      ],
                    },
                    {
                      text: "Rawfile",
                      collapsed: true,
                      link: "/asset/formats/rawfile/",
                      items: [
                        {
                          text: "Dvar List",
                          collapsed: true,
                          link: "/asset/formats/rawfile/dvar-list/",
                          items: [
                            { text: "Generic Dvar List" },
                            { text: "Shock" },
                            { text: "Sun" },
                            { text: "Vision" },
                          ],
                        },
                        {
                          text: "Graph",
                          collapsed: true,
                          link: "/asset/formats/rawfile/graph/",
                          items: [
                            { text: "Accel" },
                            { text: "Accuracy" },
                            { text: "Aim Assist" },
                            { text: "Generic Graph" },
                            { text: "Steering" },
                          ],
                        },
                        {
                          text: "Radiant",
                          collapsed: true,
                          link: "/asset/formats/rawfile/radiant/",
                          items: [
                            { text: "Client Keys" },
                            { text: "Generic Keys" },
                            { text: "Keys" },
                          ],
                        },
                        { text: "Animstatedef" },
                        { text: "Animtree" },
                        { text: "Arena" },
                        { text: "Config" },
                        { text: "Gametype Definition" },
                        { text: "Generic Rawfile" },
                        { text: "Menu Expressions" },
                        { text: "Player Animation Script" },
                        { text: "Player Animation Types" },
                        { text: "VCS" },
                      ],
                    },
                    { text: "Slug" },
                    {
                      text: "Technique Set",
                      collapsed: true,
                      link: "/asset/formats/techset/",
                      items: [{ text: "Directx 9" }, { text: "Directx 11" }],
                    },
                    {
                      text: "Video",
                      collapsed: true,
                      link: "/asset/formats/video/",
                      items: [{ text: "bik" }, { text: "WebM" }],
                    },
                  ],
                },
              ],
            },
          ],
        },
      ],
    },

    editLink: {
      pattern: "https://github.com/OpenAssetTools/Docs/edit/main/docs/:path",
    },

    socialLinks: [
      { icon: "github", link: "https://github.com/Laupetin/OpenAssetTools" },
    ],
  },
});
